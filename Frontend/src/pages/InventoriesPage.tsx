  ...
  return (
    <Layout currentPage="inventories" pageTitle="Inventories">
      ...
      {loading ? (
        ...
        <div className="bg-red-900/50 border border-red-800 rounded-xl p-6 text-center max-w-xl mx-auto my-12">
          <p className="text-red-400 font-medium mb-2">Error Loading Inventories</p>
          <p className="text-red-300 text-sm mb-4">{error}</p>
          <button
            onClick={fetchData}
            className="px-4 py-2 bg-red-800 hover:bg-red-700 text-white text-sm font-medium rounded-lg transition-colors"
          ...
          {inventoriesList.map((inventory) => (
            ...
            >
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-4 pt-4 border-t border-slate-800 text-slate-500">
                  <Calendar className="w-4 h-4" />
                  <span className="text-xs">Created {inventory.created_at ? inventory.created_at.split('T')[0] : 'N/A'}</span>
                </div>
              </div>
            </Link>
          ))}
      ...
      {isCreateOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-md w-full p-6 relative">
            <h3 className="text-lg font-semibold text-white mb-4">New Inventory</h3>
            {actionError && (
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. Main storage facility..."
                  rows={3}
                  className="w-full px-3.5 py-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all text-sm resize-none"
                />
              </div>
              <div className="flex justify-end gap-3 pt-2">
                <button
      ...
      {deletingInventory && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-md w-full p-6 relative">
            <h3 className="text-lg font-semibold text-white mb-2">Delete Inventory</h3>
            <p className="text-slate-400 text-sm mb-4">
              Are you sure you want to delete <span className="font-semibold text-white">"{deletingInventory.name}"</span>?
              This will permanently delete this inventory and all its categories and items. This action cannot be undone.
            </p>
            {actionError && (
              <p className="text-sm text-red-400 bg-red-900/30 border border-red-800 rounded-lg p-2.5 mb-4">{actionError}</p>
            )}
